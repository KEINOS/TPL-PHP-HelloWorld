...
// Path of composer.json to remove initialization test from "script"
$name_file_json_composer = 'composer.json';
$path_file_json_composer = $path_dir_package . DIR_SEP . $name_file_json_composer;

// Use the directory name as package name
$name_dir_script = basename($path_dir_package);

// Set package name to replace
$name_pkg_from = 'MyPackageName';
$name_pkg_to   = convertToUpperCamelCase($name_dir_script);

// Set repo name to replace
$name_repo_from = 'TPL-PHP-HelloWorld';
$name_repo_to   = convertToSnakeCase($name_dir_script);
...
// List of file paths to "delete" initialization files after rewriting.
$list_path_files_delete = [
    $path_dir_package . '/.devcontainer/Dockerfile.test.initialize',
    $path_dir_package . '/.devcontainer/entrypoint.test.initialize.sh',
    __FILE__
];
...
usort($list_before_after, function ($a, $b) {
    return strlen($b['before']) - strlen($a['before']);
});
 ...
 */
function dumpAutoloadComposer(): void
{
...
function getListPathFilesAll(string $path, array $list_exclude): array
{
    ...
    foreach ($patterns as $pattern) {
        foreach (\glob($pattern) as $path_file) {
            ...
            // Exclude file/dir in ignore list
...
function rewriteFileContents(string $path_file, array $list_before_after)
{
    ...
    // Rewrite strings from-to in $list_before_after
